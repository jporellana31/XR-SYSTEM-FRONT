...
export class ResonanceComponent implements OnInit {
  ...
  constructor(private diseasesService: DiseasesService, private fileService: FileService) { }
...
description += "Should use SharedArrayBuffer: " + shouldUseSharedArrayBuffer + "\n";
description += "Cross-origin isolated: " + self.crossOriginIsolated + "\n";
  if (shouldUseSharedArrayBuffer && !self.crossOriginIsolated) {
    description += "NOTE: the above configuration will NOT use SharedArrayBuffer\n";
  }
      ...
      onSelectedValueChange: (selectedValue) => {
        ...
        switch (selectedValue) {
          case blendModeOptions.MIP:
            blendModeToUse = cornerstone1.Enums.BlendModes.MAXIMUM_INTENSITY_BLEND;
            break;
          case blendModeOptions.MINIP:
            blendModeToUse = cornerstone1.Enums.BlendModes.MINIMUM_INTENSITY_BLEND;
            break;
          case blendModeOptions.AIP:
            blendModeToUse = cornerstone1.Enums.BlendModes.AVERAGE_INTENSITY_BLEND;
            break;
          default:
        ...
        toolGroup.viewportsInfo.forEach(({ viewportId, renderingEngineId }) => {
          const renderingEngine = cornerstone1.getRenderingEngine(renderingEngineId);
  ...
  const volume = await cornerstone1.volumeLoader.createAndCacheVolume(volumeId, {
    imageIds,
  });
